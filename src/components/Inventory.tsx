    ...
    <section className="work-section inventory-section" aria-labelledby="inventory-title">
      <div className="section-heading">
        ...
        <div className="heading-actions inventory-actions">
          <button className="text-button scan-action" onClick={() => setScannerOpen(true)} title="Read runes from screenshots"><ScanLine /><span className="scan-label-short">Scan</span><span className="scan-label-long">Scan screenshots</span></button>
          <button className="text-button backup-action" onClick={() => setBackupPrompt(true)} title="Save or load a local backup file"><FileDown /><span>Save / load</span></button>
      ...
      {backupPrompt && <Modal title="Personal backup" onClose={() => setBackupPrompt(false)}>
        <p className="backup-prompt-copy">Your runes are saved automatically in this browser. You do not need a file for everyday use; it is only a personal backup.</p>
        <p className="backup-prompt-copy">Loading a backup replaces the inventory currently shown in the Builder.</p>
        <div className="button-row">
          <button className="secondary-button" onClick={() => { fileInput.current?.click(); setBackupPrompt(false); }}><FileUp />Load file</button>
          <button className="primary-button" onClick={() => { onExport(); setBackupPrompt(false); }}><FileDown />Save file</button>
        </div>
      </Modal>}
    </section>
  );
